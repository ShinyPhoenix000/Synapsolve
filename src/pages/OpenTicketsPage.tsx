...
const OpenTicketsPage = () => {
  ...
  useEffect(() => {
    const fetchOpenTickets = async () => {
      let q;
      if (user?.role === 'user') {
        q = query(collection(db, 'tickets'), where('status', '==', 'open'), where('submittedBy', '==', user.uid));
      } else if (user?.role === 'agent') {
        q = query(collection(db, 'tickets'), where('status', '==', 'open'), where('assignedAgentId', '==', user.uid));
      } else {
        q = query(collection(db, 'tickets'), where('status', '==', 'open'));
      }
  ...
  return (
    <div className="relative p-6 max-w-5xl mx-auto">
      ...
      <h2 className="text-2xl font-bold text-center text-gray-900 dark:text-gray-100 mb-8">Open Tickets</h2>
      {loading ? (
        <p className="text-gray-600 dark:text-gray-300">Loading...</p>
      ) : filteredTickets.length === 0 ? (
        <p className="text-gray-500 dark:text-gray-300">No open tickets found.</p>
      ) : (
